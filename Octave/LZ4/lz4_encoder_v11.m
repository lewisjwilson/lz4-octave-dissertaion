  ...
  if length(matches{1,pos})!=0
    ...
    if(hex2dec(hi_token)>=15)
      %[hi_token hi_token_plus] = hi_lo_plus(hi_token); %run hi_lo_plus function
       hi_token_plus = [hex2dec(hi_token)-15];
        total = hi_token_plus(1);
        i=1;
        while total>=255
          "here"
          hi_token_plus(i)=255;
          total = total-255
          hi_token_plus(i+1)=total
          i++;
        end
        hi_token = dec2hex(15);
    else
    ...
    if(hex2dec(lo_token)>=15)
      %[lo_token lo_token_plus] = hi_lo_plus(lo_token); %run hi_lo_plus function
       lo_token_plus = [hex2dec(lo_token)-15];
        total = lo_token_plus(1);
        i=1;
        while total>=255
          "there"
          lo_token_plus(i)=255;
          total = total-255
          lo_token_plus(i+1)=total
          i++;
        end
        lo_token = dec2hex(15);
    else
