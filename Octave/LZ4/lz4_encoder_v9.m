...
while pos<=cols
  ...
  if length(matches{1,pos})!=0
    ...
    if(hex2dec(hi_token)>=15)
      ...
      hp_total = hi_token_plus(1);
      i=1;
      while hp_total>=255
        hi_token_plus(i)=255;
        hp_total = hp_total-255;
        hi_token_plus(i+1)=hp_total;
        i++;
      end
      hi_token = dec2hex(15);
    end
    ...
    if(hex2dec(lo_token)>=15)
      ...
      lp_total = lo_token_plus(1);
      i=1;
      while lp_total>=255
        lo_token_plus(i)=255;
        lp_total = lp_total-255;
        lo_token_plus(i+1)=lp_total;
        i++;
      end
      lo_token = dec2hex(15);
    end
...
%This is messy and a copy of something inside the loop...I dont like it, but it works
hi_token_plus = '';
if(hex2dec(hi_token)>=15)
  hi_token_plus = [dec2hex(hex2dec(hi_token)-15)];
  hp_total = hi_token_plus(1);
