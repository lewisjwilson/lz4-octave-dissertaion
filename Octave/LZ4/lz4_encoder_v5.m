...
while pos<cols
  ...
  if length(matches{1,pos})!=0
    ...
    %leave the final 5 characters unencoded
    if offset+matches{1, pos}(max_ml_idx)>=cols-5
      if offset+matches{1, pos}(max_ml_idx)==15
        pos++;
        continue;
      else
...
if length(literals)==0
  encoded_data = horzcat(encoded_data, [00 00 00 00]); %[80 97 97 97 97 97]
else
